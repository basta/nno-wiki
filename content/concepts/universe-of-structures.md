...
| $\mathrm{Pfaff}(\mathcal{R}_{\mathrm{alg}}) =: \mathcal{R}_{\mathrm{Pfaff}}$ | the **Pfaffian closure** of $\mathcal{R}_{\mathrm{alg}}$ (Speissegger): the smallest o-minimal expansion closed under solutions of Pfaffian equations / antiderivatives of definable functions. Contains $\mathcal{R}_{\exp}$. | ✓ | $\mathbb{R}$ |
...
| GELU, arctan, erf | $\mathcal{R}_{\mathrm{Pfaff}}$ | definable as antiderivatives via the [[#the-structures\|Pfaffian closure]] (Cor. 3.18) — **not** definable in $\mathcal{R}_{\mathrm{an,exp}}$ (erf: Lemma 3.14) |
